...
async function fetchUserData(userId) {
    try {
        ...
        const apiUrl = `https://social-onrender-backend-2.onrender.com/crud/particular_user/${userId}/`;
        const userResponse = await fetch(apiUrl, {
            method: "GET",
            headers: {
                Authorization: `Token ${token}`,
...
async function likePost(postId) {
    ...
    try {
        const response = await fetch(`https://social-onrender-backend-2.onrender.com/crud/posts/${postId}/like/`, {
            method: 'POST',
            headers: {
                'Authorization': `Token ${token}`,
                'Content-Type': 'application/json'
...
async function unlikePost(postId) {
    ...
    try {
        const response = await fetch(`https://social-onrender-backend-2.onrender.com/crud/posts/${postId}/unlike/`, {
            method: 'POST',
            headers: {
                'Authorization': `Token ${token}`,
                'Content-Type': 'application/json'
...
async function fetchComments(postId) {
    ...
    try {
        const response = await fetch(`https://social-onrender-backend-2.onrender.com/crud/posts/${postId}/comments/`, {
            method: 'GET',
            headers: {
                'Authorization': `Token ${localStorage.getItem('token')}`
            }
...
async function editComment(commentId, currentText) {
    ...
    if (newText && newText !== currentText) {
        ...
        try {
            const response = await fetch(`https://social-onrender-backend-2.onrender.com/crud/comments/${commentId}/`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Token ${localStorage.getItem('token')}`,
                    'Content-Type': 'application/json'
